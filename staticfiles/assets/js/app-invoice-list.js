...
$(function () {
  ...
  if (dt_invoice_table.length) {
    var dt_invoice = dt_invoice_table.DataTable({
      ajax: '/invoice_list_api/', // JSON file to add data
      scrollX: true, // Enable horizontal scrolling
      autoWidth: false, // Prevent automatic width issues
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          // Invoice ID
          targets: 2,
          className: 'all', // Prevent hiding
          render: function (data, type, full, meta) {
            var $invoice_id = full['invoice_number'];
            // Creates full output for row
            var $row_output = '<a href="/invoice-preview/' + full['invoice_number'] + '">#' + $invoice_id + '</a>';
            return $row_output;
          }
        },
        {
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $row_output =
              ...
              '<a href="/useraccount/' + full['client_id'] + '" class="text-heading text-truncate"><span class="fw-medium">' +
              $name +
              '</span></a>' +
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return '<span class="d-none">' + $total + '</span>PKR' + $total;
          }
        },
        {
          // Date
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              'data-invoice-number="' + full['invoice_number'] + '" title="Record Payment">' +
              '<i class="ti ti-pencil mx-2 ti-md"></i></button>' +
              '<button class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill" ' +
              'data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" ' +
              'data-invoice-id="' + full['invoice_number'] + '" title="Delete">' +
              '<i class="ti ti-trash mx-2 ti-md"></i></button>' +
              '<a href="/invoice-preview/' + full['invoice_number'] + '" ' +
              'class="btn btn-icon btn-text-secondary waves-effect waves-light rounded-pill" title="Preview Invoice">' +
              ...
              '<a href="/edit-invoice/' + full['invoice_number'] + '/" class="dropdown-item">Edit</a>' +
              '</div>' +
              '</div>' +
              '</div>'
            );
      ...
      dom:
        '<"row mx-1"' +
        '<"col-12 col-md-6 d-flex align-items-center justify-content-center justify-content-md-start gap-2"l<"dt-action-buttons text-xl-end text-lg-start text-md-end text-start d-flex flex-wrap gap-2"B>>' +
      ...
      buttons: [
        {
          text: '<i class="ti ti-plus ti-xs me-md-2"></i><span class="d-md-inline-block d-none mx-2">Create</span>',
        ...
        {
          text: '<i class="ti ti-download ti-xs me-md-2"></i><span class="d-md-inline-block d-none">Download</span>',
          className: 'btn btn-secondary waves-effect waves-light mx-2',
          attr: {
            'data-bs-toggle': 'modal',
            'data-bs-target': '#downloadInvoiceModal'
          }
        }
      ],
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['client_name']; // Updated to use client_name
            }
          }),
          type: 'column',
